...
const KunskapsbankArtikel = ({
  page,
  articleInfo,
}: KunskapsbankArtikelProps) => {
    ...
    <div key={page?.title}>
      ...
      <div className="contain-outer flex">
        {breadcrumbs.map((breadcrumb) => {
          const url = breadcrumb.url.replace(
            "https://spoltec-staging.h.capacedev.se",
            ""
          );
          return (
            <p
              ...
              key={breadcrumb.url}
            >
              {breadcrumb.text === breadcrumbs[0].text ? "" : "> "}
              <a href={`${url}`}>{breadcrumb.text}</a>{" "}
              {breadcrumb.text === breadcrumbs[breadcrumbs.length - 1].text
                ? ""
                : "-"}
            </p>
          );
      ...
      <div id="">
        <div className="hidden md:flex items-center absolute max-w-md md:max-w-[15rem] lg:max-w-xs md:right-10 md:top-16 lg:right-20 xl:right-20 xl:top-16 2xl:right-96 2xl:top-16 3xl:!right-[30%] z-40">
          <ul className="pl-5 list-disc block text-1-5xs article-ul">
            {categories.map((cat) => {
              ...
              return (
                <li key={cat.slug}>
                  <Link className="ul-a text-lg" href={`${url}`}>
                    {cat.title}
                  </Link>
                </li>
              );
        ...
        <div className="flex flex-col absolute right-5 bottom-28 md:bottom-20 md:right-10 2xl:right-[20%] 3xl:!right-1/4 max-w-2-5 lg:max-w-xs">
          {articleInfo[0].knappText ? (
            <p className="text-lg md:text-xl text-center pb-3">
              {articleInfo[0].knappText}
            </p>
          ) : (
            ""
...
export const getStaticPaths = async () => {
  ...
  articleNodes &&
    articleNodes?.map((node: any) => {
      if (node?.gqlArtikel?.artiklar[0]) {
        node?.gqlArtikel?.artiklar &&
          node?.gqlArtikel?.artiklar?.map((article) => {
            articleInfo.push(article);
            paths.push({
              params: { slug: article?.slug, article: article?.article },
            });
          });
      } else {
        return {
...
export const getStaticProps = async ({ params }) => {
  const data = await getPage(
    `/kunskapsbank/${params?.slug}/${params?.article}`
  );
