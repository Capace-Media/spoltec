...
const ProvtagningAvVatten = ({ data }) => {
  return (
    <div key={data?.gqlAllService?.edges[0]?.node?.title}>
      <Hero
        title={`${data?.gqlAllService?.edges[0]?.node?.title}`}
        subtitle={
          data?.gqlAllService?.edges[0]?.node?.gqlHeroFields.underrubrik
        }
        text={
          data?.gqlAllService?.edges[0]?.node?.gqlHeroFields.introduktionstext
        }
        image={
          data?.gqlAllService?.edges[0]?.node?.gqlHeroFields.bild?.mediaItemUrl
        }
      />
      <div id="content" className="w-full h-10 md:h-0"></div>
      <div id="">
        <Blocks
          blocks={data?.gqlAllService?.edges[0]?.node?.gqlBlocks?.blocks}
        />
      </div>
    </div>
  );
